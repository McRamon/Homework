...
func activate(mob: CharacterBody2D, direction: Vector2):
	...
	effect_unpacked.animation_finished.connect(_on_animation_finished)
	
	var area = effect_unpacked.get_node_or_null("attack_zone")
	if area:
		print("attack_zone detected")
		area.body_entered.connect(_on_body_entered)

func update(delta):
	if effect_unpacked and is_instance_valid(effect_unpacked):
		effect_unpacked.global_position = user.global_position
		
func _on_body_entered(body: Node2D):
	print("sword detected body: ", body)
